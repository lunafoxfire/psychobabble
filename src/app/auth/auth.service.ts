...
export class AuthService {
  ...
  public registerClient(credentials: RegisterCredentials): Observable<any> {
    const baseRequest = this.http.post('/api/auth/client/register', credentials);
    return this.interceptToken(baseRequest);
  }

  public registerSubject(credentials: RegisterCredentials): Observable<any> {
    const baseRequest = this.http.post('/api/auth/subject/register', credentials);
    return this.interceptToken(baseRequest);
  }

  public registerAdmin(email): Observable<any> {
    const baseRequest = this.http.post('/api/auth/admin/register', {email: email});
    return baseRequest;
  }

  public changeAdmin(credentials: RegisterCredentials): Observable<any> {
    const baseRequest = this.post('/api/auth/admin/change', credentials);
    return this.interceptToken(baseRequest);
  }

  public login(credentials: LoginCredentials): Observable<any> {
    const baseRequest = this.http.post('/api/auth/login', credentials);
    return this.interceptToken(baseRequest);
  }

  public verify(code: string): Observable<any> {
    if (this.isLoggedIn()) {
      const baseRequest = this.post('/api/auth/verify', {code: code});
      return this.interceptToken(baseRequest);
    }
  }
  public verifyAdmin(): Observable<any> {
    if (this.isLoggedIn()) {
      const baseRequest = this.post('/api/auth/verify-admin');
      return this.interceptToken(baseRequest);
    }
  }
  ...
  public reVerify(): Observable<any> {
    if (this.isLoggedIn()) {
      const data = this.get('/api/auth/re-verify');
      return data;
    }
  }
  ...
  public passReset(email): Observable<any> {
    const data = this.post('api/auth/reset', {email: email});
    return data;
  }

  public makeNewPass(newPass, userId, token): Observable<any> {
    const data = this.post('api/auth/change-password', {newPass: newPass, userId: userId, token: token});
    return data;
  }

  public resendPassReset(userId): Observable<any> {
    const data = this.post('api/auth/resend', {userId: userId});
    return data;
  }
