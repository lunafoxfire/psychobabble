...
export class UserService {
  ...
  public async registerAsync(regOptions: UserRegistrationOptions): Promise<User> {
    let usernameTaken = await this.findByEmailAsync(regOptions.username);
    let emailTaken = await this.findByEmailAsync(regOptions.email);
    if (!usernameTaken && !emailTaken) {
      let user = new User();
        user.username = regOptions.username;
        user.normalized_username = this.normalizeField(user.username);
        user.email = regOptions.email;
        user.normalized_email = this.normalizeField(user.email);
